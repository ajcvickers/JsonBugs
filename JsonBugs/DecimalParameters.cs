...
public static class DecimalParameters
{
    ...
    private const string TableName = "JsonScratchD1";
    ...
    private static async Task Update(SqlConnection connection2)
    {
...
param = command.CreateParameter();
param.ParameterName = "p1";
param.Value = -13579.01M;
param.DbType = DbType.Decimal;
command.Parameters.Add(param);

            command.CommandText =
                $"""
                 UPDATE [{TableName}] SET [SomeJson] = JSON_MODIFY([SomeJson], 'strict $.Decimal', @p1)
                 OUTPUT 1
                 WHERE [Id] = @p0;
                 """;
