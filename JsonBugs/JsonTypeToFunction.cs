...
public static class JsonTypeToFunction
{
    private const string DatabaseName = "JsonUpdateJsonTypeTest";
    private const string TableName = "JsonScratchJF1";
    ...
    private static async Task Query(SqlConnection sqlConnection)
    {
        ...
        await using (var command = sqlConnection.CreateCommand())
        {
            ...
            command.CommandText =
                $"""
                 UPDATE [{TableName}] SET [SomeJson] = JSON_MODIFY([SomeJson], 'strict $.OwnedReferenceBranch', JSON_QUERY(@p1))
                 OUTPUT 1
                 WHERE [Id] = @p0;
                 """;
